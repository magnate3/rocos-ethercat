...
#define ETH_ADDRESS_CABLE_RED_MASK                      (0x08) /* 1st byte of Organisationally Unique Identifier (OUI) containing the Red bit 0x08 */
#define ETH_ADDRESS_CABLE_RED_MASK_OFFSET               (0)    /* 1st byte of Organisationally Unique Identifier (OUI) containing the Red bit 0x08 */
#define ETH_ADDRESS_MASTER_RED_FORWARD_MASK             (0x04) /* 1st byte of Organisationally Unique Identifier (OUI) containing the Store and forward bit 0x04 */
#define ETH_ADDRESS_MASTER_RED_FORWARD_OFFSET           (0)    /* 1st byte of Organisationally Unique Identifier (OUI) containing the Store and forward bit 0x04 */
#define ETH_ADDRESS_FORWARDING_RULE_MASK                (0x02) /* 1st byte of Organisationally Unique Identifier (OUI) containing the Forwarding Rule bit 0x02 */
#define ETH_ADDRESS_FORWARDING_RULE_OFFSET              (0)    /* 1st byte of Organisationally Unique Identifier (OUI) containing the Forwarding Rule bit 0x02 */
#define ETH_ADDRESS_RETRY_FRAME_OFFSET                  (0)    /* 1st byte of Organisationally Unique Identifier (OUI) containing the Retry counter bits 0xF0 */
#define ETH_ADDRESS_RETRY_FRAME_MASK                    (0xF0) /* 1st byte of Organisationally Unique Identifier (OUI) containing the Retry counter bits 0xF0 */
#define ETH_ADDRESS_CABLE_RED_FRAME_ID_OFFSET           (1)    /* 2nd byte of Organisationally Unique Identifier (OUI) containing the Red Frame ID */
#define ETH_ADDRESS_CABLE_RED_FRAME_ID_MASK             (0xFF) /* 2nd byte of Organisationally Unique Identifier (OUI) containing the Red Frame ID */
#define ETH_ADDRESS_CABLE_RED_FRAME_ID_LAST             (0xFE) 
#define ETH_ADDRESS_CABLE_RED_FRAME_ID_ONLY_ONE_LINE    (0xFF) 
#define ETH_ADDRESS_LL_RESERVED_OFFSET                  (2)    /* 3rd byte of Organisationally Unique Identifier (OUI) reserved for link layer */
...
#define EC_ETHFRM_SET_RETRYINDEX(p,byVal)   EC_ETHFRM_SET_SRC_MAC_BYTE((p),ETH_ADDRESS_RETRY_FRAME_OFFSET,(EC_T_BYTE)(((EC_ETHFRM_GET_SRC_MAC_BYTE((p),(ETH_ADDRESS_RETRY_FRAME_OFFSET)) & ~ETH_ADDRESS_RETRY_FRAME_MASK) | (((byVal) & 0x0F) << 4))))
...
static EC_INLINESTART EC_T_WORD EC_ECINITCMDDESC_GET_TRANSITION(EcInitCmdDesc* p)
{
    return EC_GETWORD(
...
#if (!defined EC_NO_BITFIELDS) || (defined EC_BIG_ENDIAN)
static EC_INLINESTART EC_T_BYTE EC_ECSDOHDR_GET_CCS(EC_SDO_HDR* p)
{
    return p->uHdr.Idq.Ccs;
} EC_INLINESTOP
static EC_INLINESTART EC_T_BYTE EC_ECSDOHDR_GET_SCS(EC_SDO_HDR* p)
{
    return p->uHdr.Ids.Scs;
} EC_INLINESTOP
static EC_INLINESTART EC_T_WORD EC_ECSDOHDR_GET_INDEX(EC_SDO_HDR* p)
{
    return EC_GET_FRM_WORD((((EC_T_BYTE*)p) + EC_ECSDOHDR_OFFS_INDEX));
} EC_INLINESTOP
static EC_INLINESTART EC_T_DWORD EC_ECSDOHDR_GET_SDODATA(EC_SDO_HDR* p)
{
    return EC_GET_FRM_DWORD((((EC_T_BYTE*)p) + EC_ECSDOHDR_OFFS_SDODATA));
} EC_INLINESTOP
static EC_INLINESTART EC_T_BYTE* EC_ECSDOHDR_GET_SDODATA_PTR(EC_SDO_HDR* p)
{
...
#define EC_AOE_CMD_HDR_LEN                       sizeof(EC_AOE_CMD_HDR)
...
#define ETHERCAT_MAX_AOE_MBOX_HDR_LEN   (ETHERCAT_MBOX_HEADER_LEN + EC_AOE_HDR_LEN + EC_AOE_CMD_HDR_LEN)    
...
static EC_INLINESTART EC_T_BOOL EC_ECMBXCMDDESC_GET_TOGGLE(PEcMailboxCmdDesc p)
{
    return (*(((EC_T_PBYTE)p) + EC_ECMBXCMDDESC_OFFS_TOGGLE) != 0);
} EC_INLINESTOP
