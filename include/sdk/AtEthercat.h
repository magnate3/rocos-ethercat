...
#define EC_IOCTL_SET_SPLITTED_FRAME_PROCESSING_ENABLED          (EC_IOCTL_GENERIC | 64)
...
#define EC_IOCTL_DC_REF_CLOCK_CONTROLLED_BY_PDI     (EC_IOCTL_DC | 19)
...
#define EC_IOCTL_SB_SET_NO_DC_SLAVES_AFTER_JUNCTION \
                                                    (EC_IOCTL_SB |25)           /* 0x00050019 */
...
} EC_PACKED(8) EC_T_MASTER_RED_PARMS;
#include EC_PACKED_INCLUDESTOP
...
typedef struct _EC_T_INIT_MASTER_PARMS
{
    ...
    EC_T_LOG_PARMS EC_PACKED(8) LogParms;           /*< [in] Logging parameters */

    EC_T_MASTER_RED_PARMS EC_PACKED(8) MasterRedParms; /*< [in] Master Redundancy parameters */
...
{
    EC_T_VOID*                  pvContext;
    EC_PF_DC_STARTTIME_CB       pfnCallback;        /*< [in] If not null, DC start time calculated by application, otherwise by master */
} EC_PACKED(8) EC_T_DC_STARTTIME_CB_DESC;
#include EC_PACKED_INCLUDESTOP
...
typedef struct _EC_T_DCM_CONFIG_BUSSHIFT
{
    ...
    EC_T_BOOL                   bUseDcLoopCtlStdValues;     /*< [in] EC_TRUE don磘 change values of ESC DC time loop control register 0x930 and 0x934 */
    EC_T_DWORD                  dwInSyncStartDelayCycle;    /*< [in] delay time in msec before InSync monitoring start */
...
typedef struct _EC_T_DCM_CONFIG_MASTERSHIFT
{
    ...
    EC_T_DWORD                  dwInSyncStartDelayCycle;    /*< [in] delay time in msec before InSync monitoring start */
EC_PACKED(8) \
    EC_T_DC_STARTTIME_CB_DESC   DcStartTimeCallbackDesc;    /*< [in] If not null, DC start time calculated by application, otherwise by master */
...
typedef struct _EC_T_PROCESS_VAR_INFO_EX
{
    ...
    EC_T_WORD                   wMasterSyncUnit;                    /**< [out] Master Sync Unit */
    EC_T_WORD                   wRes1;
    EC_T_DWORD                  dwRes1;
...
typedef struct _EC_T_MASTERDEFAULTTIMEOUTS_DESC
{
    ...
    EC_T_WORD   wInitCmd;                           /* Timeout to InitCmds if no specified in ENI */
    EC_T_WORD   wReserved;
    EC_T_DWORD  dwReserved[10];
...
typedef enum _EC_T_USER_JOB
{
    ...
    eUsrJob_ProcessRxFramesByTaskId         = 9,    /*< polling mode: receive frames and process received data related to a specific task id (TaskId entry in the XML file) */
    eUsrJob_ProcessAcycRxFrames             = 10,   /*< polling mode: receive frames and process received data related to acyclic frames */
...
{
    EC_T_VOID*                  pCallbackContext;   /*< [in]  callback context  */
...
typedef union _EC_T_USER_JOB_PARMS
{
    ...
    struct
    {
        ...
        EC_T_DWORD  dwTaskId;
    } EC_PACKED(8) ProcessRxFramesByTaskId;
    struct
    {
        EC_T_DWORD  dwMaxPortsToProcess;
        EC_T_DWORD  dwNumFramesProcessed;
    } EC_PACKED(8) SwitchEoeFrames;
} EC_PACKED(8) EC_T_USER_JOB_PARMS;
#include EC_PACKED_INCLUDESTOP
...
{
    eSlaveSelectionSingle,
    eSlaveSelectionTopoFollowers,
    eSlaveSelectionMasterSyncUnit,
...
typedef struct _EC_T_SET_MAILBOX_POLLING_CYCLES_DESC
{
    EC_T_DWORD dwSlaveId;
    EC_T_WORD  wCycles;
} EC_PACKED(1)  EC_T_SET_MAILBOX_POLLING_CYCLES_DESC;
#include EC_PACKED_INCLUDESTOP
...
static EC_INLINESTART  EC_T_DWORD ecatConfigAddJunctionRedundancyConnection(
                                                    ...
                                                    EC_T_WORD               wTailRedPort    )
{
    return emConfigAddJunctionRedundancyConnection(INSTANCE_MASTER_DEFAULT, wHeadPhysAddress, wHeadRedPort, wTailPhysAddress, wTailRedPort);
} EC_INLINESTOP
...
static EC_INLINESTART  EC_T_DWORD ecatTferSingleRawCmd(EC_T_DWORD           dwSlaveId,
                                                    ...
                                                    EC_T_DWORD              dwTimeout       )
{
    return emTferSingleRawCmd(INSTANCE_MASTER_DEFAULT, dwSlaveId, byCmd, dwMemoryAddress, pbyData, wLen, dwTimeout);
} EC_INLINESTOP
...
static EC_INLINESTART  EC_T_DWORD ecatQueueRawCmd(  EC_T_DWORD              dwSlaveId,
                                                    ...
                                                    EC_T_WORD               wLen            )
{
    return emQueueRawCmd(INSTANCE_MASTER_DEFAULT, dwSlaveId, wInvokeId, byCmd, dwMemoryAddress, pbyData, wLen);
} EC_INLINESTOP
...
static EC_INLINESTART  EC_T_DWORD ecatClntQueueRawCmd(EC_T_DWORD            dwClntId,
                                                    ...
                                                    EC_T_WORD               wLen            )
{
    return emClntQueueRawCmd(INSTANCE_MASTER_DEFAULT, dwClntId, dwSlaveId, wInvokeId, byCmd, dwMemoryAddress, pbyData, wLen);
} EC_INLINESTOP
...
static EC_INLINESTART  EC_T_DWORD emBadConnectionsReset(   EC_T_DWORD      dwInstanceID     )
{
